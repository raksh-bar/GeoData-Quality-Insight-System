...
from analysis import create_findings, create_summary, defects_by_city, defects_by_type
from anomaly_detection import defective_records, duplicate_records, eta_anomalies
from data_cleaning import clean_data
from report_generator import create_dashboard, create_excel_report
...
def main() -> int:
    ...
    type_summary = defects_by_type(analysed_data)
    findings = create_findings(summary, city_summary)
    output_dir = args.output_dir.resolve()
    report_file = output_dir / "reports" / "geodata_quality_executive_report.xlsx"
    chart_file = output_dir / "charts" / "geodata_quality_dashboard.png"

    create_excel_report(
        analysed_data, summary, city_summary, type_summary, findings, duplicate_records(analysed_data),
        eta_anomalies(analysed_data), report_file,
    )
    if not city_summary.empty:
        create_dashboard(city_summary, type_summary, summary, chart_file)
