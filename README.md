# GeoData Quality Insight System

A presentation-ready Python project that checks map/location CSV data for common quality problems and produces an executive-friendly Excel report plus a dashboard chart.
...
- `reports/geodata_quality_executive_report.xlsx` — an easy-to-read, formatted management report
- `charts/geodata_quality_dashboard.png` — a presentation-ready dashboard image

## What is included in the executive report

The Excel file is designed so a manager or non-technical colleague can understand it without reading the Python code.

- **Read Me First** — report purpose, severity meanings, and an important usage note
- **Summary** — total, valid, defective records, and the data-quality score
- **Key Findings** — plain-English observations and recommended action
- **Data Dictionary** — what every input and generated column means
- **All Records** — the complete dataset with detected issue labels
- **Defects by City / Type** — where problems occur and what type they are
- **Duplicates / ETA Anomalies** — focused lists for review and correction

The PNG dashboard presents the data-quality score, valid/review counts, defects by city, defects by type, and a clear recommendation at the bottom. It is appropriate for a company meeting or portfolio demonstration.
...
```text
results/new_dataset/reports/geodata_quality_executive_report.xlsx
results/new_dataset/charts/geodata_quality_dashboard.png
```
...
1. Create a new **public** GitHub repository named `GeoData-Quality-Insight-System`. Do not initialize it with a README.
2. In this project folder, run:

```powershell
git init
git add .
git commit -m "Build GeoData Quality Insight System"
git branch -M main
git remote add origin https://github.com/YOUR_GITHUB_USERNAME/GeoData-Quality-Insight-System.git
...
> I built the GeoData Quality Insight System, a Python-based solution for checking map data. It identifies missing fields, invalid coordinates, duplicate locations, and ETA anomalies, then assigns severity to every issue. It produces an executive Excel report and a dashboard chart so non-technical teams can clearly see data-quality risks and recommended actions.

## Technologies

Python, Pandas, Pillow, OpenPyXL
